...
document.getElementById("osmFileInput").addEventListener("change", function () {
  ...
  if (!file) return;
  // se construye como formulario para luego enviar en el fetch
  ...
  fetch("http://localhost:8000/upload-graph", {
    ...
    .then((data) => {
      ...
      fetch("http://localhost:8000/graph-data")
        .then((res) => res.json())
        // objeto traído desde el back, ya convertido a objeto JS
        .then((graph) => {
          drawGraph(graph);
        });
    })
...
document
  .getElementById("pointsFileInput")
  .addEventListener("change", function () {
    ...
    fetch("http://localhost:8000/upload-points", {
      ...
      .then((data) => {
        ...
        // Llamar al backend para construir la matriz
        fetch("http://localhost:8000/build-matrix")
          .then((res) => res.json())
          .then((matrixResult) => {
            statusMessage.textContent = `Matriz lista para ${matrixResult.numPoints} puntos`;

            // El grafo ya está dibujado, solo pintamos los puntos TSP
            drawTSPPoints(data.nodeIds);
...
function renderTSPResult(result) {
  ...
  card.innerHTML = `
    ...
    <p class="stat-line"><strong>Distancia:</strong> ${result.total_cost.toFixed(
      2
    )} metros</p>
    <p class="stat-line"><strong>Tiempo:</strong> ${result.execution_time.toFixed(
      4
    )} s</p>
    <p class="stat-line"><strong>Nodos recorridos:</strong> ${result.path.join(
      ", "
    )}</p>
  `;
...
// función para pintar los nodos del TSP en color naranja
function drawTSPPoints(nodeIds) {
  // Borrar puntos TSP anteriores (las naranjas anteriores)
  window.tspPointLayers.forEach((layer) => layer.remove());
  window.tspPointLayers = [];

  if (!window.graphNodeCoords) return;

  nodeIds.forEach((id) => {
    const coords = window.graphNodeCoords[id];
    if (coords) {
